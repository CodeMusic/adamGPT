...
from espCodeMusaiAPIconnections import startApiServer, ChatCompletionRequest, ChatMessage, safe_serialize
from sample import askCodeMusai as callAskCodeMusai
...
local_iter_num = 0 # number of iterations in the lifetime of this process
raw_model = model
running_mfu = -1.0# wrap model into DDP container
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                torch.save(checkpoint, os.path.join(out_dir, 'ckpt.pt'))
                if (iter_num % 100 == 0):
                    print(f"saving sanity checkpoint to {out_dir}")
                    torch.save(checkpoint, os.path.join(out_dir, 'ckpt_sanity.pt'))
                print("\n~~~~~\n");
                callAskCodeMusai()
                cognitiveDescrepancies = estimate_loss()
                years, months = convertIterationsToYearsMonths(iter_num)
